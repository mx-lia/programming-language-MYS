...
namespace Lexer
{
	void clearSpace(unsigned char source[], int size)												// очистка ненужных пробелов и вставка разделителя строк														
	{
		char SP[] = { " ,;()[]{}=+-*/|~#<>" };
		bool findLit = false;
		int count = NULL;
	...
	LEX Analyze(In::IN in, Log::LOG log)
	{
		...
		for (int i = 0; word[i] != NULL; i++, indxLex++)											// проход по лексемам
		{
			...
			FST::FST fstNew(word[i], GRAPH_NEW);
			...
			FST::FST fstLink(word[i], GRAPH_LINK);
			if (FST::execute(fstLink))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_LINK, indexID++, line);
				LT::Add(lextable, entryLT);
				entryIT = {};
				...
				entryIT.iddatatype = LINK_TYPE;
				entryIT.value.params.count = LINK_PARAMS_CNT;
				entryIT.value.params.types = new IT::IDDATATYPE[LINK_PARAMS_CNT];
				for (int k = 0; k < LINK_PARAMS_CNT; k++)
			...
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_GONEWSTR, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}

			FST::FST fstRetrieve(word[i], GRAPH_RETRIEVE);
			if (FST::execute(fstRetrieve))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_RETRIEVE, LT_TI_NULLIDX, line);
				LT::Add(lextable, entryLT);
				continue;
			}

			FST::FST fstOutput(word[i], GRAPH_OUTPUT);
			...
			FST::FST fstUntil(word[i], GRAPH_UNTIL);
			...
			FST::FST fstWhen(word[i], GRAPH_WHEN);
			...
			if (FST::execute(fstIdentif))
			{
				...
				else
				{
					...
					if (idx != TI_NULLIDX)															// если такой идентификатор уже есть
					{
						LT::Entry entryLT = writeEntry(entryLT, LEX_ID, idx, line);
						LT::Add(lextable, entryLT);
						if (lextable.table[lextable.size - 3].lexema == LEX_NEW)
						{
				...
				if (findParm) {																		// если параметр функции или процедуры
					...
					if (lextable.table[lextable.size - 2].lexema == LEX_TYPE && lextable.table[lextable.size - 3].lexema != LEX_NEW)
						Log::WriteError(log, Error::geterrorin(305, line, position));
					if (lextable.table[lextable.size - 2].lexema == LEX_NEW && lextable.table[lextable.size - 2].lexema != LEX_TYPE)
			...
			if (FST::execute(fstOperator))
			{
				...
				case '-':
					...
					break;
				case '/':
					entryLT = writeEntry(entryLT, LEX_OPERATOR, LT_TI_NULLIDX, line, 3, LT::DIR);
					break;
				case '*':
