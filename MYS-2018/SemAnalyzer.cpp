		...
		for (int i = 0; i < lex.lextable.size; i++)
		{
			...
			case LEX_EQUAL:
			{
				...
				for (int k = i + 1; lex.lextable.table[k].lexema != LEX_SEMICOLON; k++)
				{
					if (lex.lextable.table[k].lexema == LEX_ID || lex.lextable.table[k].lexema == LEX_LITERAL ||
						lex.lextable.table[k].lexema == LEX_LINK || lex.lextable.table[k].lexema == LEX_MEASURE)
					{
						if (!ignore)
						{
			...
			case LEX_FUNCTION:
			{
				if (lex.lextable.table[i + 2].lexema == LEX_ID)
				{
					...
					{
						if (lex.lextable.table[i].lexema == LEX_RETRIEVE && (lex.lextable.table[i + 2].lexema == LEX_ID || lex.lextable.table[i + 2].lexema == LEX_LITERAL))
						{
			...
			case LEX_OPERATOR:
			{
				if (lex.lextable.table[i].operation == LT::DIR)
				{
					LT::Entry e;
					IT::Entry et;
					bool equ = false;
